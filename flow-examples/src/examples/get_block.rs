...
async fn run(account: &mut ExampleAccount, args: &mut SplitWhitespace<'_>) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let block = match args.next() {
        Some(arg) if arg.len() == 64 => {
            ...
            hex::decode_to_slice(arg, &mut block_id).map_err(|_| "Expected block height or hex encoded block ID")?;
            client.block_by_id(&block_id).await?
        }
        Some(height) => {
            let height = height.parse().map_err(|_| "Expected block height or hex encoded block ID")?;
